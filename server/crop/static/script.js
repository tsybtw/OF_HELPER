...
function toggleQueueDropdown() {
  ...
  if (!isOpening) {
    ...
    // Remember if AQ panel was open, then close it
    var aqPanel = document.getElementById('auto-queue-panel');
    var aqWasOpen = aqPanel && aqPanel.style.display !== 'none' && aqPanel.style.display !== '';
    localStorage.setItem('autoQueuePanelOpen', aqWasOpen ? 'true' : 'false');
    if (aqWasOpen) {
      var panel = document.getElementById('auto-queue-panel');
      if (panel) panel.style.display = 'none';
      clearInterval(aqScanInterval);
      clearInterval(aqSendInterval);
    }
  }
  ...
  if (dropdown.classList.contains('show')) {
    ...
    // Restore AQ panel if it was open before
    if (localStorage.getItem('autoQueuePanelOpen') === 'true') {
      openAutoQueuePanel();
    }
  }
}

function _updateQueueArrow() {
  const dropdown = document.getElementById('queue-dropdown');
  const btn = document.querySelector('.queue-toggle-btn');
...
function saveAutoQueuePanelScroll() {
  try {
    const panel = document.getElementById('auto-queue-panel');
    if (!panel) return;
    localStorage.setItem('autoQueuePanelScroll', String(panel.scrollTop || 0));
  } catch (_) { }
}

function restoreAutoQueuePanelScroll() {
  try {
    const panel = document.getElementById('auto-queue-panel');
    if (!panel) return;
    const raw = localStorage.getItem('autoQueuePanelScroll');
    if (raw != null) {
      ...
      if (Number.isFinite(val) && val >= 0) {
        ...
        requestAnimationFrame(() => {
          panel.scrollTop = val;
          setTimeout(() => { panel.scrollTop = val; }, 200);
        });
      }
    }
  } catch (_) { }
...
function updateQueueModeUI(enabled) {
  ...
  const _isOpen = document.getElementById('queue-dropdown')?.classList.contains('show');
  toggleBtn.innerHTML = `Q ${enabled ? 'ON' : 'OFF'} <span>${_isOpen ? '▲' : '▼'}</span>`;

  const autoQueueSection = document.getElementById('auto-queue-section');
    ...
    fetch(`/remove-queue-user/${userIndex}`, { method: 'POST' })
      ...
      .then(data => {
        if (data.success) {
          showStatus(data.message, 'success');
          loadInitialQueueData();
          // Refresh auto-queue list to reflect updated loaded status
          var aqPanel = document.getElementById('auto-queue-panel');
          if (aqPanel && aqPanel.style.display !== 'none') {
...
function silentlyRefreshAQList() {
  var panel = document.getElementById('auto-queue-panel');
  if (!panel || panel.style.display === 'none') return;
  fetch('/auto-queue-scan-status')
    .then(function (r) { return r.json(); })
    .then(function (data) {
      renderAQList(data.candidates || []);
      if (data.candidates && data.candidates.length > 0) {
        updateAQStartWrap(data.candidates);
      }
    }).catch(function () { });
}

function updateQueueStatus(queueData = null, browserData = null) {
  silentlyRefreshAQList();
  ...
  if (queueToggleBtn) {
    const _arrow = document.getElementById('queue-dropdown')?.classList.contains('show') ? '▲' : '▼';
    const newInnerHTML = `Q ${data.queue_mode_enabled ? 'ON' : 'OFF'} <span>${_arrow}</span>`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Resume auto-queue send polling if it was running before page reload
  if (localStorage.getItem('aqSendPending') === 'true') {
    _aqSetStopLoadingBtn();
    _aqStartSendPolling();
  }
  ...
  try {
    ...
    if (aqPanel) {
      if (aqShouldOpen) {
        aqPanel.style.display = 'block';

        if (typeof renderAQList === 'function' && typeof updateAQStartWrap === 'function') {
          fetch('/auto-queue-scan-status')
            .then(r => r.json())
            .then(data => {
              renderAQList(data.candidates || []);
              ...
              try { restoreAutoQueuePanelScroll(); } catch (_) { }
            }).catch(() => { });
        } else {
          try { restoreAutoQueuePanelScroll(); } catch (_) { }
        }
      } else {
        aqPanel.style.display = 'none';
      }

      let aqScrollSaveRaf = null;
      aqPanel.addEventListener('scroll', () => {
        if (aqScrollSaveRaf) return;
        aqScrollSaveRaf = requestAnimationFrame(() => {
          aqScrollSaveRaf = null;
          try { saveAutoQueuePanelScroll(); } catch (_) { }
        });
      }, { passive: true });
    }
  } catch (_) { }
...
var aqScanInterval = null;
var aqSendInterval = null;

function _updateScanBtnText(candidates) {
  var btn = document.getElementById('aq-scan-btn');
  if (!btn) return;
  btn.textContent = (candidates && candidates.length > 0) ? 'Rescan' : 'Scan';
}

function openAutoQueuePanel() {
  var panel = document.getElementById('auto-queue-panel');
  if (panel) panel.style.display = 'block';
  localStorage.setItem('autoQueuePanelOpen', 'true');
  fetch('/auto-queue-scan-status')
    .then(function (r) { return r.json(); })
    .then(function (data) {
      renderAQList(data.candidates || []);
      _updateScanBtnText(data.candidates);
      if (data.candidates && data.candidates.length > 0) {
        updateAQStartWrap(data.candidates);
      }
    }).catch(function () { });
...
function resetAutoQueue() {
  fetch('/auto-queue-reset', { method: 'POST' })
    .then(function () {
      var list = document.getElementById('aq-list');
      var wrap = document.getElementById('aq-start-wrap');
...
function closeAutoQueuePanel() {
  var panel = document.getElementById('auto-queue-panel');
  if (panel) panel.style.display = 'none';
  localStorage.setItem('autoQueuePanelOpen', 'false');
  clearInterval(aqScanInterval);
  clearInterval(aqSendInterval);
}

function cancelAutoQueueScan() {
  fetch('/auto-queue-scan-cancel', { method: 'POST' }).catch(function () { });
  clearInterval(aqScanInterval);
...
function _showAQClientModal(clientList, foldersEnabled) {
  ...
  overlay.id = 'aq-client-modal';
  overlay.className = 'aqd-modal';
  overlay.style.display = 'flex';

  var checks = clientList.map(function (c) {
    return '<label class="aq-client-row">' +
      '<input type="checkbox" class="aq-client-check" value="' + c.id + '" checked>' +
      '<span class="aq-client-phone">' + aqEscape(c.phone) + '</span>' +
      '</label>';
  }).join('');
...
function _doStartScan(clientIds, folderFilter) {
  ...
  var progress = document.getElementById('aq-progress');
  var list = document.getElementById('aq-list');
  var startWrap = document.getElementById('aq-start-wrap');
  var count = document.getElementById('aq-count');
  var errorDiv = document.getElementById('aq-error');
  ...
  fetch('/auto-queue-scan', {
    ...
    .then(function (data) {
      if (!data.success) {
        ...
        if (cancelBtn) cancelBtn.style.display = 'none';
        if (progress) progress.style.display = 'none';
        if (errorDiv) { errorDiv.textContent = data.message || 'Failed to start scan'; errorDiv.style.display = 'block'; }
        return;
      }
      aqScanInterval = setInterval(pollAQScanStatus, 1500);
    })
    .catch(function (e) {
      if (scanBtn) { scanBtn.disabled = false; scanBtn.style.opacity = '1'; }
      if (cancelBtn) cancelBtn.style.display = 'none';
      if (errorDiv) { errorDiv.textContent = 'Request error: ' + e; errorDiv.style.display = 'block'; }
    });
}

function pollAQScanStatus() {
  fetch('/auto-queue-scan-status')
    .then(function (r) { return r.json(); })
    .then(function (data) {
      var progressText = document.getElementById('aq-progress-text');
      var errorDiv = document.getElementById('aq-error');
      var scanBtn = document.getElementById('aq-scan-btn');
      var progress = document.getElementById('aq-progress');

      if (progressText) progressText.textContent = (data.progress || 0) + ' / ' + (data.total || 0);

      if (data.error) {
        if (errorDiv) { errorDiv.textContent = 'Error: ' + data.error; errorDiv.style.display = 'block'; }
      }
      ...
      if (!data.running) {
        clearInterval(aqScanInterval);
        var cancelBtn2 = document.getElementById('aq-cancel-btn');
        if (scanBtn) { scanBtn.disabled = false; scanBtn.style.opacity = '1'; }
...
function renderAQList(candidates) {
  var list = document.getElementById('aq-list');
  if (!list) return;

  var newHash = JSON.stringify(candidates || []);
  if (list.dataset.lastCandidates === newHash) return;
  list.dataset.lastCandidates = newHash;

  var panel = document.getElementById('auto-queue-panel');
  var oldScroll = panel ? panel.scrollTop : 0;

  list.innerHTML = '';

  if (!candidates || candidates.length === 0) {
    if (panel) panel.scrollTop = oldScroll;
    return;
  }

  candidates.forEach(function (c, idx) {
    var nickname = c.nickname || ('User ' + c.chat_id);
    var loaded = !!c.loaded;
    var included = c.included !== false;

    var container = document.createElement('div');
    container.className = 'aq-item';
    container.setAttribute('data-chat-id', c.chat_id);
    container.setAttribute('data-nickname', nickname);
    container.setAttribute('data-loaded', loaded ? 'true' : 'false');
    container.setAttribute('data-included', included ? 'true' : 'false');
...
function updateAQStartWrap(candidates) {
  var wrap = document.getElementById('aq-start-wrap');
  var count = document.getElementById('aq-count');
  var confirmBtn = document.getElementById('aq-confirm-btn');
  if (!wrap) return;
  var pending = (candidates || []).filter(function (c) {
    return c.included !== false && !c.loaded;
  });
  wrap.style.display = 'block';
  if (count) {
    var loaded = (candidates || []).filter(function (c) { return c.loaded; }).length;
    count.textContent = pending.length + ' to load' +
...
function toggleAQItem(chatId, checkbox) {
  fetch('/auto-queue-update-item', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ chat_id: chatId, included: checkbox.checked })
  }).catch(function () { });
}

// ---- AQ DIALOG MODAL ----

var _aqDialogChatId = null;
var _aqDialogOffset = 0;
var _aqDialogLoadingMore = false;

function openAQDialog(chatId) {
  _aqDialogChatId = chatId;
  _aqDialogOffset = 0;
  _aqDialogLoadingMore = false;

  // Create modal if doesn't exist
  var existing = document.getElementById('aq-dialog-modal');
  if (!existing) {
    var modal = document.createElement('div');
    modal.id = 'aq-dialog-modal';
    ...
    modal.addEventListener('click', function (e) {
      if (e.target === modal) closeAQDialog();
    });
  } else {
    existing.style.display = 'flex';
  }

  document.getElementById('aqd-pinned').innerHTML = '<div class="aqd-placeholder">Loading...</div>';
  document.getElementById('aqd-messages').innerHTML = '<div class="aqd-placeholder">Loading...</div>';
  document.getElementById('aqd-load-more').style.display = 'none';
  _fetchAQDialog(0, false);
}

function closeAQDialog() {
...
function aqDialogLoadMore() {
  if (_aqDialogLoadingMore) return;
  _aqDialogOffset += 50;
  _fetchAQDialog(_aqDialogOffset, true);
}

function _fetchAQDialog(offset, prepend) {
  if (!_aqDialogChatId) return;
  _aqDialogLoadingMore = true;
  var loadMoreBtn = document.getElementById('aqd-load-more');
  if (loadMoreBtn) loadMoreBtn.style.display = 'none';

  fetch('/auto-queue-dialog/' + _aqDialogChatId + '?offset=' + offset)
    .then(function (r) { return r.json(); })
    .then(function (data) {
      _aqDialogLoadingMore = false;
      if (data.error) {
        document.getElementById('aqd-messages').innerHTML =
          '<div class="aqd-placeholder aqd-placeholder--error">Error: ' + aqEscape(data.error) + '</div>';
        return;
      }

      // Update title with nickname
      var titleEl = document.getElementById('aqd-title');
      if (titleEl) {
        var nicknameRow = document.querySelector('#aq-list [data-chat-id="' + _aqDialogChatId + '"]');
        var nick = nicknameRow ? nicknameRow.getAttribute('data-nickname') : null;
        if (nick) titleEl.textContent = nick;
      }

      if (!prepend) {
        // Render pinned
        var pinnedEl = document.getElementById('aqd-pinned');
        if (data.pinned && data.pinned.length > 0) {
          pinnedEl.innerHTML = data.pinned.map(function (m) {
            return _aqMsgHtml(m, data.target_id);
          }).join('');
        } else {
          pinnedEl.innerHTML = '<div class="aqd-placeholder aqd-placeholder--muted">No pinned messages</div>';
        }
        // Render messages
        var msgsEl = document.getElementById('aqd-messages');
        msgsEl.innerHTML = (data.messages || []).map(function (m) {
          return _aqMsgHtml(m, data.target_id);
        }).join('');
        // Scroll to target
        setTimeout(function () {
          var target = msgsEl.querySelector('[data-is-target="1"]');
          if (target) target.scrollIntoView({ block: 'center' });
          else msgsEl.scrollTop = msgsEl.scrollHeight;
        }, 50);
      } else {
        // Prepend older messages
        var msgsEl2 = document.getElementById('aqd-messages');
        var oldScrollH = msgsEl2.scrollHeight;
        var newHtml = (data.messages || []).map(function (m) {
          return _aqMsgHtml(m, data.target_id);
        }).join('');
        msgsEl2.insertAdjacentHTML('afterbegin', newHtml);
        // Maintain scroll position
        msgsEl2.scrollTop = msgsEl2.scrollHeight - oldScrollH;
      }

      // Show load more if has_more
      var loadMoreEl = document.getElementById('aqd-load-more');
      if (loadMoreEl) loadMoreEl.style.display = data.has_more ? 'block' : 'none';
    })
    .catch(function (e) {
      _aqDialogLoadingMore = false;
      document.getElementById('aqd-messages').innerHTML =
        '<div class="aqd-placeholder aqd-placeholder--error">Request failed</div>';
    });
}

function _aqFormatText(rawText) {
  if (!rawText) return '';
  var tags = [];
  var regex = /(\[(?:photo(?: x\d+)?|media)\])/g;
  var cleaned = rawText.replace(regex, function (match) {
    tags.push(match);
    return '';
  });
  cleaned = cleaned.trim();

  var safeText = aqEscape(cleaned);
  if (tags.length > 0) {
    var tagsHtml = tags.map(function (t) {
      return '<span class="aqd-media-tag">' + aqEscape(t) + '</span>';
    }).join(' ');
    if (safeText) {
      return tagsHtml + '\n\n' + safeText;
    }
...
function _aqMsgHtml(m, targetId) {
  var isTarget = m.is_target || m.id === targetId;
  var isMe = m.direction === 'me';
  var label = isMe ? 'Me' : 'Them';

  return '<div class="aqd-msg"' +
    ' data-msg-id="' + m.id + '"' +
...
function selectAQTarget(msgId) {
  if (!_aqDialogChatId) return;
  fetch('/auto-queue-update-item', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ chat_id: _aqDialogChatId, message_id: msgId })
  }).then(function () {
    // Update target highlight in modal
    var allMsgs = document.querySelectorAll('#aqd-messages [data-msg-id], #aqd-pinned [data-msg-id]');
    allMsgs.forEach(function (el) {
      var id = parseInt(el.getAttribute('data-msg-id'));
      var isNowTarget = id === msgId;
      el.setAttribute('data-is-target', isNowTarget ? '1' : '0');
      // Update target label
      var targetLabel = el.querySelector('.aqd-msg-target-label');
      if (isNowTarget && !targetLabel) {
        var dateSpan = el.querySelector('.aqd-msg-date');
        if (dateSpan) dateSpan.insertAdjacentHTML('afterend', '<span class="aqd-msg-target-label">target</span>');
      } else if (!isNowTarget && targetLabel) {
        targetLabel.remove();
      }
    });
    ...
    fetch('/auto-queue-scan-status')
      .then(function (r) { return r.json(); })
      .then(function (d) { renderAQList(d.candidates || []); })
      .catch(function () { });
  }).catch(function () { });
}
...
function confirmAutoQueue() {
  var confirmBtn = document.getElementById('aq-confirm-btn');
  if (confirmBtn) {
    confirmBtn.disabled = false;
    confirmBtn.style.opacity = '1';
    confirmBtn.textContent = 'Stop Loading';
    confirmBtn.style.background = '#f44336';
    confirmBtn.onclick = stopAutoQueue;
  }
  localStorage.setItem('aqSendPending', 'true');
  fetch('/auto-queue-confirm', { method: 'POST' })
    .then(function () {
      _aqStartSendPolling();
    })
    .catch(function () {
      localStorage.removeItem('aqSendPending');
      _aqResetConfirmBtn();
    });
}

function _aqStartSendPolling() {
  clearInterval(aqSendInterval);
  aqSendInterval = setInterval(pollAQSendStatus, 2000);
}

function stopAutoQueue() {
  localStorage.removeItem('aqSendPending');
  fetch('/auto-queue-send-cancel', { method: 'POST' })
    .then(function () {
      clearInterval(aqSendInterval);
      _aqResetConfirmBtn();
    })
...
function _aqResetConfirmBtn() {
  var confirmBtn = document.getElementById('aq-confirm-btn');
  if (!confirmBtn) return;
  confirmBtn.disabled = false;
  confirmBtn.style.opacity = '1';
  confirmBtn.textContent = 'Start Loading';
  confirmBtn.style.background = '#4CAF50';
  confirmBtn.onclick = confirmAutoQueue;
}

function pollAQSendStatus() {
  fetch('/auto-queue-send-status')
    .then(function (r) { return r.json(); })
    .then(function (data) {
      if (data.running) {
        var count = document.getElementById('aq-count');
        if (data.total > 0 && count) {
          count.textContent = 'Loading: ' + (data.current + 1) + ' / ' + data.total;
        }
      }
      if (!data.running && data.total > 0) {
        // Send is fully complete
        clearInterval(aqSendInterval);
        localStorage.removeItem('aqSendPending');
        _aqResetConfirmBtn();
        fetch('/auto-queue-scan-status')
          ...
          .then(function (d) {
            renderAQList(d.candidates || []);
            updateAQStartWrap(d.candidates || []);
            // Count ALL loaded candidates (including previous sessions)
            var totalLoaded = (d.candidates || []).filter(function (c) { return c.loaded; }).length;
            if (totalLoaded > 0) {
              showAQDoneModal(data.queue_size_before || 0, totalLoaded);
            }
          }).catch(function () { });
      }
    })
...
function showAQDoneModal(queueSizeBefore, loadedCount) {
  // Remove existing modal if any
  var existing = document.getElementById('aq-done-modal');
  if (existing) existing.remove();

  var overlay = document.createElement('div');
  overlay.id = 'aq-done-modal';
  overlay.className = 'queue-modal';
  overlay.style.display = 'flex';
...
function aqClearOldUsers() {
  fetch('/auto-queue-clear-old', { method: 'POST' })
    .then(function (r) { return r.json(); })
    .then(function () {
      var modal = document.getElementById('aq-done-modal');
      if (modal) modal.remove();
      // Re-render candidates to reflect updated loaded status
      fetch('/auto-queue-scan-status')
        .then(function (r) { return r.json(); })
        .then(function (d) {
          renderAQList(d.candidates || []);
